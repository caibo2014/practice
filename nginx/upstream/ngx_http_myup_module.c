...
static void myup_upstream_finalize_requset(ngx_http_request_t *r, ngx_int_t rc);
...
static ngx_int_t myup_process_header(ngx_http_request_t *r) {
    ...
    for(;;) {
        h = ngx_list__push(r->upstream->headers_in.headers);
        if  (NULL ==  h){
            return NGX_ERROR;
        }
        ...
        h->value.len = r->header_end - header_start;
    ...
    if (rc = NGX_HTTP_PARSE_HEADER_DONE) {
        ...
        if(r->upstream->headers_in.date == NULL) {
            h =ngx_list__push(&->upstream->headers_in.headers);
            if(h ==NULL){
                return NGX_ERROR;
            }
